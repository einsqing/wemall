	...
	$.ajax({
    	...
    	success : function (response, stutas, xhr) {
    		...
    		$.each(json, function (index, value) {
    			...
    			$( '#wecart_'+value.cate_id + value.id +'_'+ value.price ).find('.item-image img').attr('src' , '../Uploads/'+value.img );
			});
    	}
	});
		...
		$.ajax({
			...
			success : function (response , stutas , xhr){
				...
				$.each(json , function(i , valuei){
					$.each(valuei , function(j , valuej){
						htmltitle='<div><div class="order-header"><span class="order-status">订单编号：<span class="status">'+ valuej.orderid +'</span></span></div><div class="orderResult-list" id="items-order-result"><div class="order-info"><span>订单状态：<span id="order-no">'+ valuej.status +'</span></span><span class="date" style="float: right"></span></div><div class="order-list" id="item-order-list"><ul>';
						...
						htmlbottom='</ul></div><div class="divider"></div><div class="total-info"><span>运费：<span>0.00</span>元，共<span>'+ valuej.totalprice +'</span>元</span><a class="btn dail-small">拨打电话催一催</a></div></div></div>';
					});
